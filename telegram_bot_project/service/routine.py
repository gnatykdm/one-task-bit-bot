...
class RoutineStepService:
    @staticmethod
    async def add_step(routine_id: int, step_order: int, step_description: str) -> int:
        async with get_session() as session:
            result: Any = await session.execute(
                text(
                    """
                    INSERT INTO routine_steps (routine_id, step_order, step_description)
                    VALUES (:routine_id, :step_order, :step_description)
                    RETURNING id
                    """
                ),
                {
    ...
    @staticmethod
    async def get_steps_by_routine(routine_id: int) -> List[dict]:
        async with get_session() as session:
            result: Any = await session.execute(
                text(
                    """
                    SELECT id, routine_id, step_order, step_description
                    FROM routine_steps
    ...
    @staticmethod
    async def update_step(step_id: int, step_order: Optional[int] = None,
                          step_description: Optional[str] = None) -> bool:
        async with get_session() as session:
            exists: Any = await session.execute(
                text("SELECT 1 FROM routine_steps WHERE id = :step_id"),
                {"step_id": step_id}
            )
            if not exists.scalar():
                return False
            ...
            if step_order is not None:
                update_fields["step_order"] = step_order
